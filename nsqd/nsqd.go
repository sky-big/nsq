...
// 根据配置信息创建一个nsqd服务的数据结构
func New(opts *Options) *NSQD {
	// 如果配置信息里没有设置数据存放路径，则将当前的工作目录作为数据存储路径
...
// 加载当前nsqd服务器的元数据，即当前nsqd拥有的持久的topic以及topic对应的所有channel信息
func (n *NSQD) LoadMetadata() {
	// 原子操作设置当前nsqd正在进行加载中
	atomic.StoreInt32(&n.isLoading, 1)
	defer atomic.StoreInt32(&n.isLoading, 0)

	// 打开存储当前nsqd服务器元数据的文件句柄
	...
	// 根据元数据中的topics字段得到所有的topic以及每个topic对应的channel
	for _, t := range m.Topics {
		if !protocol.IsValidTopicName(t.Name) {
			n.logf("WARNING: skipping creation of invalid topic %s", t.Name)
			continue
		}
		// 调用GetTopic接口获得对应名字的topic数据结构信息，如果不存在则重新进行创建该topic，启动的时候肯定会创建一次topic
		topic := n.GetTopic(t.Name)
		// 如果是暂停状态，则将topic设置为暂停状态
		if t.Paused {
			topic.Pause()
		}

		// 解析当前topic下的所有channel，将当前topic下的所有channel启动起来
...
// 将当前nsqd服务的topic以及每个topic对应的channel等信息写入到元数据文件中
func (n *NSQD) PersistMetadata() error {
	...
	topics := []interface{}{}
	// 组装所有的topic以及每个topic对应的channel信息
	...
	// 第一层json数据中有version版本号
	js["version"] = version.Binary
	// 第一层json数据中有topics即所有的topic信息
	js["topics"] = topics
...
// 根据topic名字得到对应的topic信息，如果不存在，则启动一个该名字的topic，然后返回该topic信息
func (n *NSQD) GetTopic(topicName string) *Topic {
	// most likely, we already have this topic, so try read lock first.
	// 从当前nsqd服务的topic地图数据结构中查找该topic
	n.RLock()
	...
	if len(lookupdHTTPAddrs) > 0 {
		// 从lookup服务中查到当前topic下的所有channel
		channelNames, _ := n.ci.GetLookupdTopicChannels(t.name, lookupdHTTPAddrs)
		// 根据获得的channel过滤掉临时的channel，然后在当前topic下启动该topic下的所有channel
	...
	// 向topic的channel更新通道发送信息，表明该topic下的channel信息有变化需要更新
	select {
...
// 消息ID生成工厂函数，不断的生成消息ID发送到idChan这个通道中，直到当前协程阻塞掉
func (n *NSQD) idPump() {
	factory := &guidFactory{}
...
//
// 当需要的worker数量超过之前分配的数量时，通过向closeCh投递消息使多余的worker销毁，
// 如果需要的数量比之前的多，则通过queueScanWorker创建新的worker
...
// queueScanWorker接收workCh发来的消息，处理，并且通过responseCh反馈消息。收到closeCh时则关闭
// 由于所有worker都监听相同的closeCh，所以当向closeCh发送消息时，随机关闭一个worker
// 且由于workCh和closeCh的监听是串行的，所以不存在任务处理到一半时被关闭的可能
// 该函数是处理延时消息的工作协程处理函数
...
func (n *NSQD) queueScanLoop() {
	...
	for {
		...
		// workTicker定时器触发扫描流程。 nsqd采用了Redis的probabilistic expiration算法来进行扫描。
		// 首先从所有Channel中随机选取部分Channel，然后遍历被选取的Channel，投到workerChan中，并且等待反馈结果，
		// 结果有两种，dirty和非dirty，如果dirty的比例超过配置中设定的QueueScanDirtyPercent，那么不进入休眠，
		// 继续扫描，如果比例较低，则重新等待定时器触发下一轮扫描。这种机制可以在保证处理延时较低的情况下减少对CPU资源的浪费
	loop:
